...
%A thing that may have the most significant impact on SOAP/BPEL performance is the strict abstraction layer, which removes (or at least makes it harder) the possibilities to use classic optimization strategies such as caching proxies. This is due to the fact that is not part of the WS-* scope and must thus be handled \emph{ad hoc} with; either a separate BPEL caching process - or outside the scope of the application. RESTful is by-design built for distributed, proxyable (and thus cacheable) operation - effectively making scalability a natural part of the development process.
