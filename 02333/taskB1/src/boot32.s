...
# An quick introduction to the AT&T syntax used by GNU Assembler can be found
# at http://sig9.com/articles/att-syntax.
#  Take help from a teaching assistant!
 ...
 # at http://www.uruk.org/orig-grub/boot-proposal.html
 .global _start
...
halt_the_machine:
 hlt						# halt the machine until next external interrupt
...
after_multiboot_header:
 ...
 pushf
 pop    %ecx
 btc    $21,%ecx		  # copy bit 21 to the carry flag and complement the bit in ecx
 jb     halt_the_machine  # bit 21 set => there is no CPUID... (jb := jump if carry is one)
 push   %ecx
 popf
 pushf
 pop    %ecx
 bt     $21,%ecx		  # copy bit 21 into carry flag
 jae    halt_the_machine  # bit 21 is reset => there is no CPUID (jae := jump if carry is zero)

 # Check if the processor is the way we assume
 # See http://www.sandpile.org/ia32/cpuid.htm for an overview of how this works
 #
...
gdt_32_descriptors:
 ...
 # Descriptor for the TSS (not used until Task 3)
TSS_descriptor:
 .quad  0, 0
 ...
 # section 5.1 (pp. 115-117) for an explanation of the page translation mechanism.
 .align 4096
pml4_base:
