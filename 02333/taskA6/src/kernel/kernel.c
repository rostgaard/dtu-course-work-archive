...
condition_variable_table[MAX_NUMBER_OF_CONDITION_VARIABLES];
...
initialize(void) {
    ...
    /* Loop over all semaphores in the semaphore table and reset the parent. */
    for (i = 0; i < MAX_NUMBER_OF_SEMAPHORES; i++) {
        semaphore_table[i].calling_process = -1;
    }

    /* Loop over all mutexes in the mutexe table and reset the parent. */
    for (i = 0; i < MAX_NUMBER_OF_MUTEXES; i++) {
        mutex_table[i].calling_process = -1;
    }

    /* Loop over all mutexes in the mutexe table and reset the parent. */
    for (i = 0; i < MAX_NUMBER_OF_CONDITION_VARIABLES; i++) {
        condition_variable_table[i].owner = -1;
    }
