...
\begin{tabular}{| l | l | l |}
  ...
  \hline
  2 & [int A[n]]$^\ell$ & f$_\ell^I (\widehat{\sigma}) = \widehat{\sigma}[$A[i]$ \mapsto [0,0] | \forall i \in \{0, ..., n-1 \}$\\
  \hline
  3 & [x := a]$^\ell$ & f$_\ell^I (\widehat{\sigma}) = \widehat{\sigma}[x \mapsto \mathcal{A}_I[\![a]\!] \widehat{\sigma} ]$ \\
  \hline
  4 & [skip]$^\ell$ & f$_\ell^I (\widehat{\sigma}) = \widehat{\sigma}$\\
  \hline
  5 & [b]$^\ell$ & f$_\ell^I (\widehat{\sigma}) = \widehat{\sigma}$\\
  \hline
  6 & [int A[a$_1$] := a$_2$]$^\ell$ & f$_\ell^I (\widehat{\sigma}) = \widehat{\sigma}[$A[a$_1$]$ \mapsto \mathcal{A}_I[\![a_2]\!] \widehat{\sigma} ]$ \\
  \hline
  7 & [read x]$^\ell$ & f$_\ell^I (\widehat{\sigma}) = [x \mapsto \mathcal{A}_I[\![n]\!] \widehat{\sigma})$  \\
  \hline
  8 & [read A[a]]$^\ell$ & f$_\ell^I (\widehat{\sigma}) = \widehat{\sigma}[$A[a]$ \mapsto \mathcal{A}_I[\![n]\!] \widehat{\sigma} ]$ \\
  \hline
