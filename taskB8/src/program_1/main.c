...
main(int argc, char* argv[]) {
    ...
    if (0 >= ipc_thread_stack || 0 >= timer_thread_stack) {
        printat(MAIN_DEBUG_LINE, 0, "Light Control: Could not allocate the thread's stack!\n");
        return;
    }
...
void IPC_handler_thread(void) {
    ...
    if (recv_port < 0) {
        printat(IPC_DEBUG_LINE, 0, "Light Control IPC: findport_process_failed");
        return;
    }
    ...
    while (1) {
        if ((ALL_OK != receive(recv_port, &msg, &sender, &type)) ||
            ...
            if (ALL_OK != semaphoredown(local_mutex)) {
                printat(IPC_DEBUG_LINE, 0, "Light Control IPC: semaphoredown failed!\n");
                break;
            } else {
            ...
            if (ALL_OK != semaphoreup(local_mutex)) {
                printat(IPC_DEBUG_LINE, 0, "Light Control IPC: semaphoreup failed!\n");
                break;
            } else {
...
void timer_thread() {
    ...
    while (1) {
        ...
        /* Enter critical region */
        if (ALL_OK != semaphoredown(local_mutex)) {
            printat(TIMER_DEBUG_LINE, 0, "Light Control Timer: semaphoredown failed!\n");
            break;
        } else {
        ...
        if (ALL_OK != semaphoreup(local_mutex)) {
            printat(TIMER_DEBUG_LINE, 0, "Light Control Timer: semaphoreup failed!\n");
            break;
        } else {
